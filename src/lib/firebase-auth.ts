...
class FirebaseAuthManager {
  ...
  async initializeAuth() {
    try {
      ...
      this.showError('認証システムの初期化に失敗しました: ' + (error as Error).message)
    }
  }

  // IDトークンをサーバーに送信
  async sendTokenToServer(idToken: string) {
    try {
  ...
  showSpinner(buttonId: string, show: boolean) {
...
export { authManager, FirebaseAuthManager }